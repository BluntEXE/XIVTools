...
- **Modpack Operations** — install `.ttmp2` / `.pmp` files, upgrade pre-Dawntrail mods, convert between formats, create distributable `.pmp` packages
- **Texture Viewer** — preview any `.tex` file from the game or a mod, export as PNG or DDS
- **Texture Import** — replace game textures; imports PNG or DDS into a mod
- **Material Editor** — edit `.mtrl` files: colorset rows, dye templates, color pickers for all channels
- **Model Viewer** — preview any `.mdl` file with textures, rendered in OpenGL
...
- **File Override Inspector** — see all files in the active collection, which mod provides each, and at what priority
...
2. Extract:
   ```bash
   unzip XIVTools-linux-v{version}.zip -d ~/XIVTools
   ```
...
Click **Save and Apply**. XIVTools reinitializes the game cache.
...
Lists all mods in the active Penumbra collection. Each row shows the mod name, folder name, enabled state, and priority.

Toggle the switch next to a mod to enable or disable it. XIVTools writes the change to Penumbra's collection file. If the game is running with Penumbra active, the change applies live via Penumbra's HTTP API.
...
| **Create Modpack** | Any mod folder | `.pmp` ready to share |

Drag a modpack file onto the window, or use **Browse** to open one.

### Texture Viewer

Open a `.tex` file to preview it. The viewer shows a full-resolution preview with zoom and pan, the format, dimensions, and mip count. Export using the **Export PNG** or **Export DDS** buttons.

Click **Open File** to browse for a `.tex` file, or open one from the **Item Browser**.
...
- **Colorset** — 16 rows, each with diffuse, specular, emissive, gloss, and metalness values. Click any color swatch to open the color picker.
...
Click **Open .mdl** to load a model. XIVTools loads textures from the same mod directory.

### Model Import

1. Click **Open .mdl** and select the game model to replace
2. Click **Import Model** and select your file (`.obj`, `.fbx`, `.glb`, `.gltf`, or `.dae`)
3. Review the mesh list. XIVTools maps each group in your file to one mesh in the game model.
...
XIVTools reads bone weights from the file if present. Skinned characters require bone names to match the original skeleton exactly.

OBJ import has no extra dependencies. FBX, glTF, and DAE require `libassimp` on Linux; on Windows it's bundled.

### Item Browser

1. Type in the search box to filter by item name
2. Select an item from the list
3. Click **Texture**, **Material**, or **Model** to open it in the corresponding viewer

XIVTools builds the item database from the game's Excel sheets on first run. Search is instant once the index loads.
...
Lists all file overrides in the active Penumbra collection: the game file path, the mod providing it, that mod's priority, and whether the mod is enabled.

If two mods override the same file, the higher-priority mod wins.
...
cd XIVTools/xivtools-ui
dotnet run -c Release
```

The patched xivModdingFramework source is at `../lib/xivModdingFramework`. The build uses a local `ProjectReference`; no separate setup needed.
